...
def get_dipole_vs_interface_angles(frame,substrate,sampling_indices=None,):


    if sampling_indices is None:
        sampling_indices = np.arange(len(frame))
    ...
    sampling_indices = [i for i in water_O_indices if i in sampling_indices]

    voronoi_dict = frame_analyser.get_voronoi_dict(sampling_indices)

    interface_dipole_angles = {}

    for atom_index in sampling_indices:
