    ...
    for sim_dir_key in trajectory_dirs.keys():
        ...
        for key, dirs in grouped_simulation_details.items():
            ...
            for i,traj_name in enumerate(dirs):
                ...
                traj = ase.io.read(traj_dir + '/' + traj_name + '.xyz',index=traj_sampling_freq)     
            ...
            for i , traj in enumerate(trajectories):
                ...
                if randomly_choose_frames:
                    print(f"Randomly sampling {num_frames_to_sample_per_traj} frames from trajectory {traj_names[i]}")
                    print(f"Num frames to sample:", num_frames_to_sample_per_traj)
                    print(f"Num frames available:", num_frames)
                ...
                for frame_index in frame_indices_to_sample:
                    ...
                    # print('cell before changes:', cell)
                    ...
                    fhi_aims_input_file_tools.make_FHI_AIMS_calc_dir(frame,
                                                                    frame_calc_dir,
                                                                    time_hrs=time_hrs,
                                                                    basis_sets_dir=basis_sets_dir,
                                                                    default_control_file_path=default_control_file_path,
                                                                    default_slurm_file_path=default_slurm_file_path,
